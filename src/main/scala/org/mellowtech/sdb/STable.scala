...
abstract class STable[A : Ordering] {
  ...
  def insert[B](key: A, cn: ColumnName, v: B): STable.this.type =  row(key) match {
    case None => this + (Some(key), SRow(Map(cn -> v)))
    case Some(r) =>
      val ra = r + (cn, v)
      this + (Some(key), ra)
  }
...
object STable {
  ...
  def autoIncKey[A](key: Option[A], t: STable[A]): A = t.header.genKey match {
    case true if key.isEmpty => t.header.incrHighId.asInstanceOf[A]
    case _ => key.get
  }
